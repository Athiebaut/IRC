 ...
 *
 * If the format of the channel name is invalid, and the optional parameter `opt` is set to "PRINT",
 * it sends an error message to the client indicating the bad channel name.
 *
 * If the channel does not exist, and the optional parameter `opt` is set to "PRINT",
 ...
 * @param opt Optional parameter to specify if error messages should be sent to the client ("PRINT").
 ...
 */
int IrcHelper::isRightChannel(const Client& client, const std::string& channelName, std::map<std::string, Channel*>& channels, const std::string& opt)
{
 ...
 * @param start_pos The position in the string to start searching from.
 ...
 */
int IrcHelper::findCharFromPosition(const std::string& str, char target1, char target2, size_t start_pos)
{
 ...
 */
size_t IrcHelper::howManyArgsIsWaiting(std::string mode)
{
	size_t size = 2;
	
	if (mode.find('o') != std::string::npos)
		size++;
	if (mode.find('l') != std::string::npos && mode[findCharFromPosition(mode, '-', '+', mode.find('l'))] == '+')
		size++;
 ...
 */
std::map<char, std::string> IrcHelper::whichModeForWhichArg(std::vector<std::string> args)
{
	...
	for (size_t i = 0; i < mode.size(); i++)
	{
		...
		{
			if (mode[findCharFromPosition(mode, '-', '+', i)] == '+')
			{
 ...
 */
void IrcHelper::checkDuplicate(std::string &str, char c, size_t i)
{
	i++;
	while (str[i])
	{
